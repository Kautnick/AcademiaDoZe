...
public class ColaboradorService : IColaboradorService
{
    ...
    public ColaboradorService(IColaboradorRepository repository, IPasswordHasher hasher)
    {
    ...
    public async Task<ColaboradorDto> AdicionarAsync(ColaboradorDto dto, string senha, CancellationToken cancellationToken = default)
    {
    ...
    {
        var domainTipo = tipo switch
        {
            AppColaboradorTipo.Administrador => ColaboradorTipo.Administrador,
            AppColaboradorTipo.Instrutor => ColaboradorTipo.Instrutor,
            _ => ColaboradorTipo.Atendente
        };
        var list = await _repository.ObterPorTipo(domainTipo, cancellationToken);
        return list.Select(c => c.ToDto());
    }
    ...
    {
        var domainVinc = vinculo switch
        {
            AppColaboradorVinculo.CLT => ColaboradorVinculo.CLT,
            AppColaboradorVinculo.Estagio => ColaboradorVinculo.Estagio,
            _ => ColaboradorVinculo.CLT
        };
        var list = await _repository.ObterPorVinculo(domainVinc, cancellationToken);
        return list.Select(c => c.ToDto());
    }
